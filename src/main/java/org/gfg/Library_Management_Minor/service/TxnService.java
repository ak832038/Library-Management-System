...
import org.gfg.Library_Management_Minor.repository.TxnRepository;
...
public class TxnService {
    ...
    public User getUserFromDB(TxnRequest txnRequest) throws TxnException {
        // student is correct ?
        User userFromDb = userService.getStudentByPhoneNo(txnRequest.getUserPhoneNo(), UserType.STUDENT);
        if(userFromDb == null){
    ...
    public Book getBookFromDB(TxnRequest txnRequest) throws TxnException {
        List<Book> books = bookService.findBookListDynamically(FilterType.BOOK_NO, Operator.EQUALS, txnRequest.getBookNo());
        if(books.isEmpty()){
